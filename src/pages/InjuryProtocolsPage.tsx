...
export default function InjuryProtocolsPage() {
  ...
  const title = isAr ? '????? ?????????? ????????' : 'Injury Protocol Library';
        ...
        <div className="space-y-8 not-prose">
          <section className="rounded-[2rem] border border-slate-200 bg-slate-50 p-6 sm:p-8">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-health-green/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-health-green">
              ...
              <span>{isAr ? '????? ?????? ?????' : 'Practical rehab library'}</span>
            </div>
            <h2 className="mb-3 text-2xl font-black text-slate-900 sm:text-3xl">
              {isAr ? '????? ?????? ??? ????? ?????? ????? ???????' : 'Dedicated pages for each injury protocol'}
            </h2>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              >
                {isAr ? '???? ??????? ?? ????? ?????' : 'Ask the assistant about an injury'}
              </Link>
            </div>
          </section>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 text-sm font-bold text-slate-900">
                ...
                <span>{isAr ? '???? ????? ????????' : 'Search and filter injuries'}</span>
              </div>
              {hasActiveFilters ? (
                <button
                  type="button"
            ...
            <div className="space-y-4">
              <input
                ...
                placeholder={isAr ? '???? ???? ??????? ?? ??????? ?????' : 'Search by injury or medical term'}
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">
                  ...
                  <span>{isAr ? '??????? ???????' : 'Current filters'}</span>
                </div>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center gap-2 text-sm font-bold text-slate-900">
              ...
              <span>{isAr ? '????????? ????????' : 'Main categories'}</span>
            </div>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <div className="text-sm font-bold text-slate-900">{isAr ? '????? ????????' : 'Injury pages'}</div>
                <p className="mt-1 text-sm text-slate-500">
                  {isAr ? `??? ??????? ???????: ${filtered.length}` : `Current matching results: ${filtered.length}`}
                </p>
              </div>
              {loading ? <div className="text-sm text-slate-400">{isAr ? '???? ???????...' : 'Loading...'}</div> : null}
            </div>

            {filtered.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">
                {isAr ? '?? ???? ????? ?????? ????.' : 'No matching injury pages right now.'}
              </div>
            ) : (
