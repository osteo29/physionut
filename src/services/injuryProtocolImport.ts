...
export type ImportedExercisePlan = {
  label: string;
  sets?: string;
  cues?: string[];
};
...
export type ImportedPhase = {
  ...
  exercises: string[];
  exercisePlans: ImportedExercisePlan[];
};

export type ImportedInjuryProtocol = {
  heading: string;
  sourceTitle: string;
...
export type ProtocolImportSummary = {
  ...
  matched: ImportedInjuryProtocol[];
  unmatched: ImportedInjuryProtocol[];
};
...
function buildCandidates(injuries: InjuryProtocol[]): InjuryCandidate[] {
  return injuries.map((protocol) => ({
    id: protocol.id,
    name: protocol.name,
...
function parseExerciseRow(row: string): ImportedExercisePlan | null {
...
function parsePhaseLines(lines: string[]) {
  ...
  for (let index = 0; index < phaseHeaders.length; index += 1) {
    ...
    const exercisePlans = exerciseRows.map(parseExerciseRow).filter((value): value is ImportedExercisePlan => Boolean(value));
...
export function parseInjuryProtocolText(rawText: string, injuries: InjuryProtocol[] = getAllInjuries()): ImportedInjuryProtocol[] {
  const candidates = buildCandidates(injuries);

  return splitInjuryBlocks(rawText)
    .map<ImportedInjuryProtocol>((block) => {
...
export function summarizeImportedProtocols(injuries: ImportedInjuryProtocol[]): ProtocolImportSummary {
...
export function renderInjuryProtocolOverridesTS(injuries: ImportedInjuryProtocol[]) {
...
export function renderInjuryProtocolImportReport(injuries: ImportedInjuryProtocol[], outputPath: string) {
  const summary = summarizeImportedProtocols(injuries);
  const lines = [
    `Parsed injuries: ${summary.parsedCount}`,
    `Matched injuries: ${summary.matchedCount}`,
    `Unmatched injuries: ${summary.unmatchedCount}`,
    `Output: ${outputPath}`,
    '',
    'Matched:',
    ...summary.matched.map((injury) => `- ${injury.sourceTitle} -> ${injury.matchedSlug} (${injury.phases.length} phases)`),
    '',
    'Unmatched:',
    ...summary.unmatched.map((injury) => `- ${injury.heading}`),
  ];

  return `${lines.join('\n')}\n`;
}

export function getImportedProtocolSlugSet(injuries: ImportedInjuryProtocol[]) {
  return new Set(injuries.map((item) => item.matchedSlug).filter((item): item is string => Boolean(item)));
}
