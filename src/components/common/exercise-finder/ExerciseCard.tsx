...
function highlightText(text: string, query: string): ReactNode {
  ...
  return parts.map((part, i) =>
    regex.test(part) ? (
      <mark key={i} className="bg-health-green/20 text-health-green-dark rounded px-0.5 not-italic">
        {part}
      </mark>
    ) : (
      part
  ...
  return (
    <article className={`medical-card h-full border-l-4 p-5 sm:p-6 ${difficultyBorderMap[exercise.level]}`}>
      <div className="flex h-full flex-col gap-4">
        ...
        <p className="text-sm leading-7 text-slate-600">{highlightText(exercise.description, searchQuery)}</p>
